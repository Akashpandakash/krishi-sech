...
export const cropRepository = process.env.DATABASE_URL
  ? new PrismaCropRepository(prisma)
  : appConfig.appEnv === "production"
    ? (() => {
        throw new Error("DATABASE_URL is required in production");
      })()
...
export const calendarTaskRepository = process.env.DATABASE_URL
  ? new PrismaCalendarTaskRepository(prisma)
  : appConfig.appEnv === "production"
    ? (() => {
        throw new Error("DATABASE_URL is required in production");
      })()
