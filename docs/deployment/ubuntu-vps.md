...
Use Ubuntu 24.04 LTS with Nginx, PostgreSQL, Node 20 LTS, and systemd. This option requires a named operator for patching, monitoring, backup restoration, and incident response.
...
- Do not expose application port 3000 or PostgreSQL 5432 through UFW/security groups.
- Bind Node to `127.0.0.1:3000`; bind local PostgreSQL to loopback.
...
- Migrate once: `npm run migrate:deploy`
...
The systemd unit must use `WorkingDirectory=/srv/krishi-sech/current`, load the external environment file, restart on failure with a bounded delay, set `NoNewPrivileges=true`, and grant writes only where required. Use `After=network-online.target postgresql.service` for local PostgreSQL. Set `TRUST_PROXY=true` because exactly one trusted Nginx proxy fronts Express.
...
- Run daily encrypted PostgreSQL logical backups with a least-privileged backup identity, transfer them off-server, and apply agreed daily/monthly retention. Take a snapshot before every migration.
...
3. Run `npm ci`, `npm test`, and `npm run build` as the service user.
4. Snapshot PostgreSQL and run `npm run migrate:deploy` once.
...
3. Leave backward-compatible schema changes and ship a forward fix. For an incompatible migration, stop writes, restore the pre-deploy snapshot into a new database, update the external `DATABASE_URL`, restart, and verify readiness.
