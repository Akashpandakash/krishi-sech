...
export function loadAppConfig(
  ...
  // OPENAI_ENABLED is the pre-Gemini name for this switch; still honored.
  const aiEnabled = booleanValue(
    values,
    values.AI_ENABLED === undefined ? 'OPENAI_ENABLED' : 'AI_ENABLED',
    true,
  );
  const aiProvider = (values.AI_PROVIDER?.trim() || 'gemini') as AiProviderName;
  ...
  if (production && aiEnabled) {
    const keyName =
      aiProvider === 'gemini' ? 'GEMINI_API_KEY' : 'OPENAI_API_KEY';
    if (!values[keyName]?.trim()) {
      throw new Error(`${keyName} is required when AI_PROVIDER=${aiProvider}`);
    }
  }
  return {
    appEnv,
